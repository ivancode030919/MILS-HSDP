...
")
    ...
    Public Sub loadLVInventory(lv As ListView, area As String, id As String, des As String, com As String)
        ...
        If newHome.roleId = 20020 Or newHome.roleId = 20021 Then
            ...
            SQL.ExecQueryDT("SELECT TOP 50 '' AS dummy,T0.id,T0.goodDes,sum(ISNULL(T1.qty,0)) AS QTY,
                            CASE
	                            WHEN T0.Status = 1 THEN 'Active'
	                            WHEN T0.Status = 2 THEN 'Inactive'
	                            WHEN T0.Status = 3 THEN 'Discontinued'
                            END AS StatusDisplay
                            FROM tblGoods T0 
                            LEFT JOIN tblProducts T1 ON T0.id=T1.goodId
                            WHERE (T0.id <> 0 ) AND
                            (T0.id = @id or @id = '')
                            AND (T0.goodDes LIKE @des or @des ='')
                            AND (T0.Status = @stat or @stat ='')
                            GROUP BY t0.id,t0.goodDes,T0.Status
                            ORDER BY T0.id")
            If SQL.HasException(True) Then Exit Sub
            lv.Items.Clear()
            lv.View = View.Details
    ...
    Public Sub loadLVRelsDetails(id As String)
        SQL.AddParam("@id", id)
        SQL.ExecQueryDT("SELECT T0.id,T0.goodId,T1.goodDes,T2.batchName,T3.name,T0.qty,COALESCE(T0.Expirationdate, 'N/A') AS Expirationdate,T0.pId,T2.id,t3.id,T0.qty AS BASE,'0' as adjstqty
    ...
    Public Sub loadLVBatchesView(lv As ListView, tbx As TextBox)
        SQL.AddParam("@flter", tbx.Text)
        SQL.ExecQueryDT("SELECT * FROM tblbatches WHERE (batchName = @flter or @flter='') ORDER BY batchName asc")
        If SQL.HasException(True) Then Exit Sub
        lv.Items.Clear()
        lv.View = View.Details
        lv.FullRowSelect = True
    ...
    Public Sub loadLVLocationsView(lv As ListView, tbx As TextBox, aId As String, rId As String)
        ...
        If rId = "20016" Or rId = "20017" Or rId = "20018" Then
            ...
            SQL.ExecQueryDT("SELECT *
                                FROM tblLocations
                                WHERE ([name] = @flter or @flter='') AND areaid = @aId 
                                ORDER BY
                                  CASE
        ...
        Else
            SQL.AddParam("@flter", tbx.Text)
            SQL.AddParam("@aId", aId)
            SQL.ExecQueryDT("SELECT * FROM tblLocations WHERE ([name] = @flter or @flter='')
                        AND areaId =  @aId;")
            If SQL.HasException(True) Then Exit Sub
            lv.Items.Clear()
            lv.View = View.Details
    ...
    Public Sub loadLVLocationsViewadmin(lv As ListView, tbx As String, aId As String)
        ...
        SQL.ExecQueryDT("SELECT TOP 50 t0.id,t0.name,t0.remarks,t0.lastModified,t1.area
                         ...
                         WHERE (t0.name = @flter or @flter ='') AND (t1.area = @aId or @aId ='')
                            ORDER BY
                                CASE
                                WHEN ISNUMERIC(name) = 1 THEN REPLICATE('0', 100 - LEN(name)) + name
                                ELSE name
...
from tblAreas ")
    ...
    Public Sub validatematch(id As String)
        SQL.AddParam("@id", id)
        SQL.ExecQueryDT("SELECT id FROM tblGoods WHERE id = @id")
        If SQL.HasException(True) Then Exit Sub
        With goodsAutoBuild
            If SQL.RecordCountDT <> 0 Then
                .match = True
    ...
    Public Sub loadGoodsListLV(lv As ListView, tbx1 As String, tbx2 As String)

        SQL.AddParam("@id", tbx1)
        SQL.AddParam("@des", "%" & tbx2 & "%")
        SQL.ExecQueryDT("SELECT top 100 * FROM tblGoods
                        WHERE (id = @id or @id='')
    ...
    Public Sub loadGoodsRegisterHeader(lv As ListView, entryId As String, strDate As Date, stpDate As Date)
        SQL.AddParam("@id", entryId)
        SQL.AddParam("@str", strDate)
        SQL.AddParam("@stp", stpDate)

        SQL.ExecQueryDT("SELECT T0.id AS ID,
                         T0.encDate AS [ENCODED DATE],
                         T1.name +' '+T1.lastname AS ENCODER,
                         T0.tag AS [REMARKS],
                         T0.date AS DATE 
                         FROM tblGoodsABHeaders T0 
                         INNER JOIN tblEmpDetails T1 ON T0.encby=T1.id 
                         WHERE (T0.encDate BETWEEN @str AND @stp) AND (T0.id = @id OR @id='')
    ...
    Public Sub fetchDatasInventoryAdjustDetails(lv As ListView, id As String)
        ...
        SQL.ExecQueryDT("SELECT T0.id, T0.rmks, T0.userName, T0.transDate,
                         (SELECT CONCAT(CASE WHEN name IS NULL THEN '' ELSE name +' ' END,
                                        CASE WHEN middlename IS NULL THEN '' ELSE middlename +' ' END,
                                        CASE WHEN lastname IS NULL THEN '' ELSE lastname +' ' END)  
                                    FROM tblEmpDetails WHERE id = T0.approvBy), t0.rmks1
...
FROM tblProductsABHeaders T1
    ...
    Public Sub addandcheckIP(ipadd As String)
        Dim stat As String = "Open"
        SQL.AddParam("@ip", ipadd)
        SQL.ExecQueryDT("SELECT ipadd FROM tblUsed WHERE ipadd LIKE '%' + @ip +'%'")
        If SQL.HasException(True) Then Exit Sub

        If SQL.RecordCountDT = 0 Then

            SQL.AddParam("@ip", ipadd)
            SQL.AddParam("@stat", stat)
            SQL.ExecQueryDT("INSERT INTO tblUsed(ipadd,status) VALUES (@ip,@stat)")
            If SQL.HasException(True) Then Exit Sub
        Else
            SQL.AddParam("@ip", ipadd)
            SQL.AddParam("@stat", stat)
            SQL.ExecQueryDT("UPDATE tblUsed SET status = @stat WHERE ipadd = @ip")
            If SQL.HasException(True) Then Exit Sub
        End If
    End Sub

    Public Sub addandcheckIPwhenclose(ipadd As String)
    ...
    Public Sub checkarea(ipadd As String)
        SQL.AddParam("@ip", ipadd)
        SQL.AddParam("@stat", newHome.areaId)
        SQL.ExecQueryDT("UPDATE tblUsed SET locatin = @stat WHERE ipadd = @ip")
    End Sub
