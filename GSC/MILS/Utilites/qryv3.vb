...
Public Class qryv3
    ...
    Public Sub relupdate(id As String, t1 As String, t2 As Date, t3 As String, t4 As String, t5 As String, t6 As String, t7 As String, t8 As String)
        ...
        '        SQL.AddParam("@id", .Rows(i).Cells(0).Value.ToString)
        '        SQL.AddParam("@gId", .Rows(i).Cells(1).Value.ToString)
        '        SQL.AddParam("@loc", .Rows(i).Cells(9).Value.ToString)
        '        SQL.AddParam("@batch", .Rows(i).Cells(8).Value.ToString)
        ...
        '        SQL.ExecQueryDT("UPDATE tblRelsDetails 
        '                        SET pId=@pId
        '                        ,goodId=@gId
        '                        ,areaId=@area
        '                        ,locId=@loc
        ...
        '        With releaseDetails.dg1
        '            For i As Integer = 0 To .Rows.Count - 1
        '                SQL.AddParam("@ajst", .Rows(i).Cells(11).Value.ToString)
        '            Next
        '        End With
...
ORDER BY T0.tileID;
    ...
    'UPDATE RECEIVE DETAILS ACTION -only SPU
    '=================================================================================================================================================================================
    Public Sub updateTransferheader(HEADERID As Integer, TDATE As Date, REMARKS As String, DOCUM As String)

        Dim CURRENTUSER As String = newHome.userId
        Dim EDITDATE As Date = Date.Now
        MsgBox(CURRENTUSER)

        SQL.AddParam("@USERID", CURRENTUSER)
        SQL.ExecQueryDT("SELECT T2.name + ' ' + T2.lastname AS EMPLOYEE FROM tblUsers T1
                 INNER JOIN tblEmpDetails T2 ON T1.empId = T2.id
                 WHERE T1.empId = @USERID")

        If SQL.HasException(True) Then Exit Sub

        If SQL.RecordCountDT > 0 Then

            CURRENTUSER = SQL.DBDT.Rows(0)("EMPLOYEE").ToString()

        Else

        End If

        SQL.AddParam("@EDITDATE", EDITDATE.ToString("MM/dd/yyyy"))
        SQL.AddParam("@EDITOR", CURRENTUSER)
        SQL.AddParam("@HEADERID", HEADERID)
        SQL.AddParam("@TDATE", TDATE)
        SQL.AddParam("@REMARKS", REMARKS)
        SQL.AddParam("@DOCNUM", DOCUM)

        SQL.ExecQueryDT("UPDATE tblIntraTransferHeaders SET dateTransfer =@TDATE, remarks_1 = @REMARKS +':EDITED: '+@EDITOR+' : '+@EDITDATE,transferId=@DOCNUM
                         WHERE id = @HEADERID")
        If SQL.HasException(True) Then Exit Sub
    ...
    '=================================================================================================================================================================================
    'UPDATE BUILD HEADER ACTION -only SPU
    '=================================================================================================================================================================================
    Public Sub updateBuildheader(HEADERID As Integer, TDATE As Date, REMARKS As String, OWNERSHIP As String, DOCUMENTNO As String)
        ...
        SQL.AddParam("@TDATE", TDATE)
        SQL.AddParam("@REMARKS", REMARKS)
        SQL.AddParam("@OWNERSHIP", OWNERSHIP)
        SQL.AddParam("@DOCUMENTNO", DOCUMENTNO)

        SQL.ExecQueryDT("UPDATE tblProductsABHeaders SET transDate =@TDATE,
                        rmks = @REMARKS +':EDITED: '+@EDITOR+' : '+@EDITDATE,
                        rmks_1 = @OWNERSHIP,docNum = @DOCUMENTNO WHERE
                        id=@HEADERID")
        If SQL.HasException(True) Then Exit Sub
    ...
    '=================================================================================================================================================================================
    'UPDATE COUNT HEADER ACTION -only SPU
    '=================================================================================================================================================================================
    Public Sub updateCOUNTheader(HEADERID As Integer, TDATE As Date, REMARKS As String, DOCNUMBER As String)
        ...
        SQL.AddParam("@TDATE", TDATE)
        SQL.AddParam("@REMARKS", REMARKS)
        SQL.AddParam("@DOCNUMBER", DOCNUMBER)

        SQL.ExecQueryDT("UPDATE tblInventoryAdjustHeaders SET transDate =@TDATE,
                        rmks = @REMARKS +':EDITED: '+@EDITOR+' : '+@EDITDATE,rmks1=@DOCNUMBER
					    WHERE id=@HEADERID")
        If SQL.HasException(True) Then Exit Sub
